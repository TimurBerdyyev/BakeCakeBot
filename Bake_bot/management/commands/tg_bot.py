...
def make_cake(update: Update, context):
    ...
    if user_input == 'Ваши заказы':
        ...
        for order in orders:
            update.message.reply_text(
            f'Заказ {order.order_number}: цена {order.order_price} руб., статус "{order.order_status}",'  # Гдето тут ошибка
...
def send_order_2(update: Update, context: CallbackContext):
    ...
    if user_input == 'Да':
        ...
        if context.user_data['Срочность'] == 'Срочно':
            total_price = int(cake.price * 1.2)
        else:
        ...
        create_new_order_2(update.message.chat_id, temp_order, total_price)
